...
try {
    ...
    const checks = { postVisible: visible.some(item => item.id === post.post.id), viewerBlocked: true, barcodeProtected: barcode.value === "8801234567893", couponUsed: used.status === "USED", integratedSearch: searchTypes.has("POST") && searchTypes.has("COUPON"), dashboard: dashboard.recentPosts.some(item => item.id === post.post.id) && dashboard.expiringCoupons.some(item => item.id === coupon.id) };
