...
공간에서 초대를 만들면 입력한 주소로 초대 메일 발송을 시도합니다. 기본 Docker 환경에서는 외부 메일로 바로 보내지 않고 Mailpit(`http://localhost:8025`)에서 안전하게 확인합니다. 실제 이메일 주소로 발송하려면 `.env`에 사용하는 SMTP 서비스의 `MAIL_HOST`, `MAIL_PORT`, `MAIL_USERNAME`, `MAIL_PASSWORD`, `MAIL_SMTP_AUTH`, `MAIL_STARTTLS_ENABLED`, `MAIL_FROM` 값을 설정합니다. 초대와 일반 활동 알림은 동일한 공통 발송 계층과 발신 주소를 사용합니다.
