...
- Mailpit 기반 로컬 메일과 선택적 Gmail SMTP 초대 메일
...
공간에서 초대를 만들면 입력한 주소로 초대 메일 발송을 시도합니다. 기본 Docker 환경에서는 외부 메일로 바로 보내지 않고 Mailpit(`http://localhost:8025`)에서 안전하게 확인합니다. 실제 이메일 주소로 발송하려면 `.env`에 사용하는 SMTP 서비스의 `MAIL_HOST`, `MAIL_PORT`, `MAIL_USERNAME`, `MAIL_PASSWORD`, `MAIL_SMTP_AUTH`, `MAIL_STARTTLS_ENABLED`, `MAIL_FROM` 값을 설정합니다.
...
1~10단계와 12단계의 인증, 공간, 고급 캘린더, 공유함, 쿠폰함, 알림·설정, 통합검색·대시보드, 세션·초대·공간 생명주기, 감사 기록, 일정-자료 연결과 로컬 운영 안정화 기능을 사용할 수 있습니다. 11단계 기능 간 상세 화면 연결은 아직 예정입니다. 이메일은 기본 설정에서 Docker의 Mailpit으로 전달되며, `.env`에 외부 SMTP를 설정하면 실제 수신자에게 발송됩니다.
