...
| 영역 | 구현 내용 |
| --- | --- |
| 인증 | 이메일 회원가입·로그인, 비밀번호 해시, 1시간 JWT, 세션 복원, 계정 삭제 |
...
- Flyway 마이그레이션 13개로 데이터 구조 변경 이력을 관리합니다.
...
상용 배포, 소셜 로그인, 비밀번호 재설정, 웹 푸시와 오브젝트 스토리지는 아직 구현하지 않았습니다. 현재는 로컬 실행과 기능 검증에 초점을 둔 포트폴리오 단계이며, 이메일은 DB Outbox·자동 재시도·관리자 이력까지, 상세 화면 딥링크는 오늘 화면·검색·알림·연결 자료까지 적용했습니다.
