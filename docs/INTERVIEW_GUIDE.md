...
- Flyway 마이그레이션 12개로 데이터 구조 변경 이력을 관리합니다.
...
상용 배포, 소셜 로그인, 비밀번호 재설정, 웹 푸시, 메일 Outbox·재시도와 오브젝트 스토리지는 아직 구현하지 않았습니다. 현재는 로컬 실행과 기능 검증에 초점을 둔 포트폴리오 단계이며, 상세 화면 딥링크는 오늘 화면·검색·알림·연결 자료까지 적용했습니다.
